...
def run_pipeline(img_path: Path):
    ...
    TABLE_OUTPUT_PATH = OUTPUT_DIR / f"table_{img_path.stem}.jpg"
    LABEL_PATH = OUTPUT_DIR / "labels" / f"{img_path.stem}.txt"
    CROPPED_OUTPUT_PATH = OUTPUT_DIR / f"cropped_{img_path.stem}.jpg"
    STRUCT_OUTPUT_PATH = OUTPUT_DIR / f"struct_{img_path.stem}.jpg"
    EXTRACTED_JSON_PATH = OUTPUT_DIR / f"extracted_{img_path.stem}.json"
    ...
    results = model.predict(
        source=preprocessed.output_path,
        conf=0.8, 
        save_txt=True, 
    ...
    if image is None:
        print(f"Skipped unreadable image: {preprocessed.output_path}")
        sys.exit(1)
    ...
    if lines[0]:
        ...
        if x2 <= x1 or y2 <= y1:
            logger.warning(f"Invalid bounding box for {TABLE_OUTPUT_PATH}: ({x1}, {y1}, {x2}, {y2})")
            exit(1)

        cropped = image[y1:y2, x1:x2]
        if cropped.size == 0:
            logger.warning(f"Invalid cropped image for {TABLE_OUTPUT_PATH}: ({x1}, {y1}, {x2}, {y2})")
            exit(1)

        cv2.imwrite(str(CROPPED_OUTPUT_PATH), cropped)
        logger.info(f"Saved: {CROPPED_OUTPUT_PATH}")
    ...
    Path(EXTRACTED_JSON_PATH).write_text(
        json.dumps(
            {
                "image file:": str(CROPPED_OUTPUT_PATH),
    ...
    logger.info("Table JSON saved: %s", EXTRACTED_JSON_PATH)
