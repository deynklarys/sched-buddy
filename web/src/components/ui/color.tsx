...
function SliderTrack({ className, ...props }: AriaSliderTrackProps) {
  return (
    <AriaSliderTrack
      className={composeRenderProps(className, (className) =>
        cn('border-border h-7 w-[192px] rounded-md border', className),
      )}
      {...props}
    />
  )
...
function ColorSwatchPicker({ className, ...props }: AriaColorSwatchPickerProps) {
  return (
    <AriaColorSwatchPicker
      className={composeRenderProps(className, (className) =>
        cn('flex flex-wrap gap-1', className),
      )}
      {...props}
    />
  )
...
function ColorSwatchPickerItem({ className, ...props }: AriaColorSwatchPickerItemProps) {
  return (
    <AriaColorSwatchPickerItem
      className={composeRenderProps(className, (className) =>
        cn(
          'ring-offset-background size-8 overflow-hidden rounded-lg border-0 transition-colors',
          /* Selected */
