...
function SelectTrigger({
  ...
  const dataClasses =
    'data-[placeholder]:text-muted-foreground data-[size=default]:h-fit data-[size=sm]:h-8'
  const childrenClasses =
    "[&_svg:not([class*='text-'])]:text-muted-foreground *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
  const transitionClasses = 'transition-[color,box-shadow]'
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          'bg-popover text-popover-foreground rounded-xl border shadow-sm',
          position === 'popper' &&
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        childrenClasses,
        textBodyClassNames,
        'relative flex w-full cursor-default items-center gap-2 rounded-lg py-1.5 pr-8 pl-2 outline-hidden select-none',
        className,
      )}
      {...props}
    >
